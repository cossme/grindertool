...
class Connection(object):
    ...
    def __init__(self, host=None, port=None, sample_rate=None, disabled=None):
        ...
        logger.info( '*** connecting to STATSD: host=%s,port=%d' % (self._host, self._port))
        self.udp_sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        self.udp_sock.connect((self._host, self._port))
        logger.info('Initialized connection to %s:%d with P(%.1f)' % (self._host, self._port, self._sample_rate))
    ...
    def send(self, data, sample_rate=None):
        ...
        '''
        if self._disabled:
            logger.info( 'python-statsd: Connection disabled, not sending data')
            return False
        ...
        try:
            ...
            logger.error('python-statsd: unexpected error %r while sending data' % (e))
            return False
